...
d_2019 <- read_excel("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", skip = 3) %>% 
  ...
  # the .x means this dataset, the "course_" is what we
  # are replacing, and then the "" is what it is replaced 
  # with (IE nothing).
...
read_enrollment <- function(file, y = 3) {
  x <- read_excel(file, skip = y) %>% 
  clean_names() %>% 
  rename_with(~ str_replace(.x, "course_", "")) %>%
  select(id, title, name, department, u_grad) %>%
  drop_na() %>% 
...
d_2018 <- read_enrollment("raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", y = 2)
